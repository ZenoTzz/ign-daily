...
function appData() {
  return {
    ...
    automationConfig: {
      ...
      api_model: 'gpt-6-sol',
      api_title_model: 'gpt-6-sol',
      api_fulltext_model: 'gpt-6-sol',
      api_nightly_model: 'deepseek-v4-flash',
      api_title_thinking: 'low',
      api_fulltext_thinking: 'low',
    ...
    preferredModelLabel(art) {
      if (art?.translation_status === 'requested') {
        return this.isApiMode('fulltext_translator')
          ? this.formatTranslatorModel(this.automationConfig.api_fulltext_model || 'gpt-6-sol')
          : `${this.fulltextQueueOwner()} 待处理`;
      }
      if (art?.translation_status === 'done') {
        return this.translatorLabel(art) || '已完成';
      }
      return this.formatTranslatorModel(this.automationConfig.api_title_model || this.automationConfig.api_model || 'gpt-6-sol');
    },
    ...
    async loadAutomationConfig() {
      try {
        ...
        this.automationConfig = {
          ...
          api_model: cfg.api_model || 'gpt-6-sol',
          api_title_model: cfg.api_title_model || cfg.api_model || 'gpt-6-sol',
          api_fulltext_model: cfg.api_fulltext_model || 'gpt-6-sol',
    ...
    async saveAutomationConfig() {
      try {
        ...
        const cfg = {
          ...
          api_provider: 'openai-compatible',
          api_model: this.automationConfig.api_title_model || this.automationConfig.api_model || 'gpt-6-sol',
          api_title_model: this.automationConfig.api_title_model || this.automationConfig.api_model || 'gpt-6-sol',
          api_fulltext_model: this.automationConfig.api_fulltext_model || 'gpt-6-sol',
    ...
    apiTranslationInputs() {
      ...
      const titleModel = this.apiModelById(this.automationConfig.api_title_model || this.automationConfig.api_model || 'gpt-6-sol');
      const fulltextModel = this.apiModelById(this.automationConfig.api_fulltext_model || 'gpt-6-sol');
      const inputs = {
        fulltext_limit: '5',
    ...
    sourceImages(source) {
      ...
      const cover = source?.cover_image ? [source.cover_image] : [];
      return [...new Set([...cover, ...images]
