...
- 英文和中文之间不留空格。
- 引用用「」，作品名用《》。
- 金额写成 `500美元(约合人民币金额)` 这种格式，具体数值不要使用固定旧汇率。
...
- 网页设置会写 `data/automation-config.json`：`title_translator`、`fulltext_translator` 可设为 `openclaw` 或 `api`；`nightly_learner` 还可设为 `codex`。
...
- `nightly_learner=codex` 时，API workflow 和 OpenClaw nightly 都必须跳过。Codex 定时任务按 `AGENT_NIGHTLY_LEARNER.md` 执行：分析原译与润色差异，沿用同一证据池和周报确认机制；只有周报反馈明确采纳时才写入 `STYLE_PROFILE.md`。
