...
for article in idx["articles"]:
    index_text = "\n".join(
        str(article.get(key) or "") for key in ("cn_title", "subtitle", "summary")
    )
    bad_quotes = disallowed_double_quotes(index_text)
    if bad_quotes:
        rendered = " ".join(f"U+{ord(char):04X}" for char in bad_quotes)
        errors.append(
            f"#{article.get('id')}: non-corner double quote in index Chinese text ({rendered}); use「」"
        )
...
for aid in done_ids:
    ...
    if not subtitle:
        ...
        if len(subtitle) > 15:
            errors.append(
                f"#{aid}: subtitle TOO LONG ({len(subtitle)}字 > 15字上限): 「{subtitle}」"
            )
        elif len(subtitle) < 2:
        ...
        if "，" in subtitle and len(subtitle) > 10:
            warnings.append(
                f"#{aid}: subtitle 含逗号且偏长，建议确认是否仍像短句: 「{subtitle}」"
            )
        ...
        if (
            len(subtitle) > 8
            and not any(c in subtitle for c in "！？!?~")
            and "的" in subtitle
            and "了" in subtitle
        ):
            warnings.append(
                f"#{aid}: subtitle 读起来像完整句子，建议更口语化: 「{subtitle}」"
            )
    ...
    if not data.get("opus_summary"):
        ...
        if summary_len < 60 or summary_len > 110:
            errors.append(
                f"#{aid}: opus_summary LENGTH {summary_len}, target 70-80, allowed 60-110"
            )
    ...
    for txt in cn_texts:
        ...
        if bad_quotes:
            rendered = " ".join(f"U+{ord(char):04X}" for char in bad_quotes)
            errors.append(
                f"#{aid}: non-corner double quote in Chinese text ({rendered}); use「」"
            )
            break
        ...
        if isinstance(p, dict):
            ...
            for amount, currency in money_patterns:
                if "人民币" not in cn and "元)" not in cn:
                    warnings.append(
                        f"#{aid}: {amount}{currency} may lack CNY conversion"
                    )
                    break
