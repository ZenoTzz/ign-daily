...
def health() -> dict[str, Any]:
    return {"ok": True, "time_cn": now_cn(), "app_dir": str(APP_DIR)}
...
def replace_dict(payload: DictReplaceRequest, user: sqlite3.Row = Depends(current_user)) -> dict[str, Any]:
    gh_put_file("data/dict.json", json_text(payload.dictionary), payload.message)
    sync_from_github()
...
def add_dict_term(payload: DictTermRequest, user: sqlite3.Row = Depends(current_user)) -> dict[str, Any]:
    ...
    gh_put_file("data/dict.json", json_text(dictionary), message)
    sync_from_github()
...
def request_translation(payload: TranslationRequest, user: sqlite3.Row = Depends(current_user)) -> dict[str, Any]:
    ...
    gh_put_file(path, json_text(updated), f"request translation for {payload.date}: {','.join(map(str, payload.ids))}")
